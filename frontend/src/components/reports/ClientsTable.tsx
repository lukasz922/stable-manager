...
export function ClientsTable({ period }: Props) {
  const [items, setItems] = useState<ClientReport[]>([]);
  ...
  useEffect(() => {
    async function load() {
      try {
        setLoading(true);
        setError("");
        setItems(await getClientsReport(period));
      } catch (err) {
        setError(
    ...
    >
      ...
      {loading ? (
        ...
        <Box sx={{ py: 5, textAlign: "center" }}>
          <Typography color="text.secondary">Brak jazd klientów w tym okresie.</Typography>
        </Box>
      ) : (
        <Stack spacing={1.5}>
